...
static SBN_Status_t ProcessLocalSub(CFE_SB_MsgId_t MsgID, CFE_SB_Qos_t QoS)
{
    ...
    {
        EVSSendErr(SBN_SUB_EID, "local subscription ignored for MsgID 0x%04X, max (%d) met", CFE_SB_MsgIdToValue(MsgID),
    ...
    for (NetIdx = 0; NetIdx < SBN.NetCnt; NetIdx++)
    {
        ...
        for (PeerIdx = 0; PeerIdx < Net->PeerCnt; PeerIdx++)
        {
            ...
            EVSSendDbg(SBN_PEER_EID, "process local sub for MID %#04x sending to %d:%d", CFE_SB_MsgIdToValue(MsgID),
...
SBN_Status_t SBN_CheckSubscriptionPipe(void)
{
    ...
    switch (CFE_Status)
    {
        case CFE_SUCCESS:
            ...
            {
                EVSSendErr(SBN_MSG_EID, "unexpected message id (0x%04X) on SBN.SubPipe", CFE_SB_MsgIdToValue(MsgId));
                return SBN_ERROR;
            } /* end switch */
